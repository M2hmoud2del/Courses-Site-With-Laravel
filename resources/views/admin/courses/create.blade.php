...
<div class="card">
    <div class="card-content">
        <form action="{{ route('admin.courses.store') }}" method="POST" style="max-width: 800px; margin: 0 auto;">
            @csrf
            ...
            <div class="form-row" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                <div class="form-group">
                    ...
                    <input type="number" id="price" name="price" class="form-input" placeholder="Enter price" step="0.01" value="{{ old('price') }}" required>
                    @error('price') <span class="error-text">{{ $message }}</span> @enderror
                </div>
                
                <div class="form-group">
                    <label for="duration">Duration (weeks)</label>
                    <input type="number" id="duration" name="duration" class="form-input" placeholder="Enter duration in weeks" value="{{ old('duration') }}">
                </div>
            </div>
