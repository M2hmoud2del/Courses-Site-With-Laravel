...
<div class="card">
    <div class="card-content">
        <form action="{{ route('instructor.courses.update', $course->id) }}" method="POST" style="max-width: 800px; margin: 0 auto;">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <div class="mode-description">
                    ...
                    <span id="request-mode-desc" style="display: {{ $course->is_closed ? 'block' : 'none' }};">
                        ...
                        Students must send join requests that require your approval
                    </span>
                </div>
                <input type="hidden" id="is_closed" name="is_closed" value="{{ old('is_closed', $course->is_closed) }}">
            </div>
            
            <div class="course-stats" style="margin-top: 2rem; padding: 1rem; background: var(--gray-50); border-radius: 0.5rem; border: 1px solid var(--gray-200);">
                <h4 style="margin-bottom: 0.5rem; font-size: 0.875rem; color: var(--gray-700);">Course Statistics</h4>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem;">
                    <div>
                        <label style="display: block; font-size: 0.75rem; color: var(--gray-600);">Total Students</label>
                        <p style="font-weight: 600; color: var(--blue-600);">{{ $course->enrollments_count ?? 0 }}</p>
                    </div>
                    <div>
                        <label style="display: block; font-size: 0.75rem; color: var(--gray-600);">Average Progress</label>
                        @php
                            $avgProgress = $course->enrollments->avg('progress') ?? 0;
                        @endphp
                        <p style="font-weight: 600; color: var(--green-600);">{{ round($avgProgress, 1) }}%</p>
                    </div>
                    <div>
                        <label style="display: block; font-size: 0.75rem; color: var(--gray-600);">Total Revenue</label>
                        <p style="font-weight: 600; color: var(--purple-600);">
                            ${{ number_format($course->price * ($course->enrollments_count ?? 0), 2) }}
                        </p>
                    </div>
                </div>
            </div>
            
            <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid var(--gray-200);">
